...
void WrapMdUser::On(const FunctionCallbackInfo<Value>& args) {
    ...
    if (args[0]->IsUndefined() || args[1]->IsUndefined()) {
        ...
        ThrowException(Exception::TypeError(String::New("Wrong FunctionCallbackInfo<Value>->event name or function")));
...
void WrapMdUser::Connect(const FunctionCallbackInfo<Value>& args) {
    ...
    if (args[0]->IsUndefined()) {
        ...
        ThrowException(Exception::TypeError(String::New("Wrong FunctionCallbackInfo<Value>->front addr")));
        args.GetReturnValue().Set(Undefined(isolate));
        return;
    }    
    int uuid = -1;
...
void WrapMdUser::ReqUserLogin(const FunctionCallbackInfo<Value>& args) {
    ...
    if (args[0]->IsUndefined() || args[1]->IsUndefined() || args[2]->IsUndefined()) {
        ...
        ThrowException(Exception::TypeError(String::New("Wrong FunctionCallbackInfo<Value>")));
                args.GetReturnValue().Set(Undefined(isolate));
        return;
    }
...
void WrapMdUser::pkg_cb_userlogin(CbRtnField* data, Local<Value>*cbArray) {
    ...
    *(cbArray + 1) = Boolean::New(data->bIsLast)->ToBoolean();
    ...
    if (pRspUserLogin) {
        ...
        jsonRtn->Set(String::NewFromUtf8(isolate,"TradingDay"), String::New(pRspUserLogin->TradingDay));
        jsonRtn->Set(String::NewFromUtf8(isolate,"LoginTime"), String::New(pRspUserLogin->LoginTime));
        jsonRtn->Set(String::NewFromUtf8(isolate,"BrokerID"), String::New(pRspUserLogin->BrokerID));
        jsonRtn->Set(String::NewFromUtf8(isolate,"UserID"), String::New(pRspUserLogin->UserID));
        jsonRtn->Set(String::NewFromUtf8(isolate,"SystemName"), String::New(pRspUserLogin->SystemName));
        ...
        jsonRtn->Set(String::NewFromUtf8(isolate,"MaxOrderRef"), String::New(pRspUserLogin->MaxOrderRef));
        jsonRtn->Set(String::NewFromUtf8(isolate,"SHFETime"), String::New(pRspUserLogin->SHFETime));
        jsonRtn->Set(String::NewFromUtf8(isolate,"DCETime"), String::New(pRspUserLogin->DCETime));
        jsonRtn->Set(String::NewFromUtf8(isolate,"CZCETime"), String::New(pRspUserLogin->CZCETime));
        jsonRtn->Set(String::NewFromUtf8(isolate,"FFEXTime"), String::New(pRspUserLogin->FFEXTime));
        jsonRtn->Set(String::NewFromUtf8(isolate,"INETime"), String::New(pRspUserLogin->INETime));
        *(cbArray + 2) = jsonRtn;
    }
    else {
...
void WrapMdUser::pkg_cb_rspsubmarketdata(CbRtnField* data, Local<Value>*cbArray) {
    ...
    *(cbArray + 1) = Boolean::New(data->bIsLast)->ToBoolean();
    ...
    if (pSpecificInstrument) {
        Local<Object> jsonRtn = Object::New();
        jsonRtn->Set(String::NewFromUtf8(isolate,"InstrumentID"), String::New(pSpecificInstrument->InstrumentID));
        *(cbArray + 2) = jsonRtn;
    }
    else {
        *(cbArray + 2) = Local<Value>::New(Undefined());
...
void WrapMdUser::pkg_cb_rtndepthmarketdata(CbRtnField* data, Local<Value>*cbArray) {
    ...
    if (pDepthMarketData) {               
        Local<Object> jsonRtn = Object::New();
        jsonRtn->Set(String::NewFromUtf8(isolate,"TradingDay"), String::New(pDepthMarketData->TradingDay));
        jsonRtn->Set(String::NewFromUtf8(isolate,"InstrumentID"), String::New(pDepthMarketData->InstrumentID));
        jsonRtn->Set(String::NewFromUtf8(isolate,"ExchangeID"), String::New(pDepthMarketData->ExchangeID));
        jsonRtn->Set(String::NewFromUtf8(isolate,"ExchangeInstID"), String::New(pDepthMarketData->ExchangeInstID));
        ...
        jsonRtn->Set(String::NewFromUtf8(isolate,"UpdateTime"), String::New(pDepthMarketData->UpdateTime));
        ...
        jsonRtn->Set(String::NewFromUtf8(isolate,"AveragePrice"), Number::New(pDepthMarketData->AveragePrice));tf
        jsonRtn->Set(String::NewFromUtf8(isolate,"ActionDay"), String::New(pDepthMarketData->ActionDay));           
        *cbArray = jsonRtn;
    }
    else {
...
Local<Value> WrapMdUser::pkg_rspinfo(CThostFtdcRspInfoField *pRspInfo) {
    if (pRspInfo) {
        ...
        jsonInfo->Set(String::NewFromUtf8(isolate,"ErrorMsg"), String::New(pRspInfo->ErrorMsg));
        return jsonInfo;
    }
    else {
